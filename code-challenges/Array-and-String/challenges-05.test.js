...
const listFoods = (recipe) => {
  ...
  for ( let i = 0 ; i < recipe.ingredients.length ; i ++){
    let removeNumbers = recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(' ')+1)
    let removeAmount = removeNumbers.slice(removeNumbers.indexOf(' ')+1);
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  for (let i=0 ; i < recipe.ingredients.length ; i++) {
    let removeNumbers = recipe.ingredients[i].split(' ');
    removeNumbers.shift();
    removeNumbers.shift();
    let afterRemove= removeNumbers.join(' ');
    result.push(afterRemove);
  }
...
const stepActions = (recipe) => {
  ...
  for (let i=0 ; i < recipe.steps.length ; i++) {
    let firstWord = recipe.steps[i].slice(0,recipe.steps[i].indexOf(' '));
